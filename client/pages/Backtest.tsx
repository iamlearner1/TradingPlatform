...
export default function Backtest() {
  ...
  // Simulate a live market ticking Option Chain & Payoff Graph!
  useEffect(() => {
  ...
  const chartData = useMemo(() => {
    ...
    for(let i = minS; i <= maxS; i += 25) {
        let totalPl = 0;   // Expiry P&L
        let t0Pl = 0;      // Live T+0 P&L — shifts with every spot tick
        ...
        activePositions.forEach(p => {
            ...
            // ── T+0 P&L: fair-value at hypothetical spot i, minus live entry price at current spot ──
            // Entry price at current spot is re-derived each tick so graph shifts with market
  ...
  const stats = useMemo(() => {
    if (positions.length === 0) return { livePl: 0, maxP: '₹0.00', maxL: '₹0.00', be: [] };
    const livePl = chartData.find(d => d.spot >= spotPrice)?.t0 || 0;
    const maxP = Math.max(...chartData.map(d => d.pl));
    const maxL = Math.min(...chartData.map(d => d.pl));
    
    const be: number[] = [];
    for(let i=1; i<chartData.length; i++) {
        if((chartData[i-1].pl < 0 && chartData[i].pl > 0) || (chartData[i-1].pl > 0 && chartData[i].pl < 0)) {
            be.push(chartData[i].spot);
        }
    }
  ...
  const optionChain = useMemo(() => {
     ...
     const chain = [];
     for(let i = ATM - 300; i <= ATM + 300; i += 50) {
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden max-w-md mx-auto relative shadow-2xl bg-[#f8f9fa]">
      {/* HEADER */}
      <div className="bg-white flex items-center px-4 py-3 border-b border-gray-100 flex-shrink-0 justify-between z-[90]">
        <div className="flex items-center gap-2">
           <Link to="/strategy" className="p-1.5 -ml-1.5 rounded-full hover:bg-gray-100 transition-colors">
             <ArrowLeft className="w-5 h-5 text-gray-700" />
           </Link>
           <span className="text-[16px] font-bold text-gray-900 tracking-tight ml-1">Strategy Builder</span>
        </div>
      ...
      <div className="flex-1 overflow-y-auto bg-gray-50 flex flex-col">
                         ...
                         const QtyPicker = ({leg, type, action}: {leg: any, type: any, action: any}) => leg ? (
                           <div style={{display:'flex',alignItems:'center',background:'#f3f4f6',borderRadius:'6px',padding:'2px'}}>
                             <button type="button" onClick={() => adjustPositionQty(r.s, type, action, -1)}
                               style={{width:24,height:24,borderRadius:'4px',background:'#fff',boxShadow:'0 1px 2px rgba(0,0,0,0.05)',fontWeight:800,fontSize:14,cursor:'pointer',color:'#374151',border:'none',display:'flex',alignItems:'center',justifyContent:'center'}}>-</button>
                         ...
                         return (
                           <tr key={r.s} style={{borderBottom:'1px solid #f9fafb',background:isAtmRow?'#fcfdfd':'#fff',verticalAlign:'top'}}>
                             ...
                             <td className="py-2.5 px-1 align-top">
                                <div className="flex justify-center relative w-[60px] mx-auto">
                                   <div className={`border rounded-[6px] font-extrabold text-[12px] w-full h-[28px] flex items-center justify-center ${isAtmRow ? 'bg-blue-50 border-blue-200 text-blue-700' : 'bg-gray-50 border-gray-100 text-gray-700'}`}>{r.s}</div>
                                </div>
                             </td>
                             <td className="py-2.5 px-1 align-top text-center">
          ...
          <div className="bg-white mt-1 border-t border-gray-200 flex-shrink-0 pb-10">
                <div className="p-3 border-b border-gray-100 flex items-center justify-between bg-[#f8f9fa]">
                    <h2 className="font-black text-[14px] text-gray-900 uppercase tracking-widest text-[#1e1b4b]">Net Strategy Payoff</h2>
                    <button onClick={() => setPositions([])} className="text-[10px] font-bold text-red-600 bg-red-50 px-2.5 py-1 rounded">CLEAR ALL</button>
                </div>
                
                {/* Stats Grid */}
                <div className="grid grid-cols-2 lg:grid-cols-3 p-4 gap-y-4 gap-x-2 bg-white">
                   <div>
                ...
                {/* Graph */}
                <div className="bg-white px-2 py-6 border-t border-gray-100 relative">
                   <ResponsiveContainer width="100%" height={260}>
                      <ComposedChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                        ...
                        <XAxis dataKey="spot" type="number" domain={['dataMin', 'dataMax']} tick={{fontSize: 10, fill: '#6b7280', fontWeight: 'bold'}} tickLine={false} axisLine={{stroke: '#e5e7eb'}} tickFormatter={(val) => Math.round(val).toLocaleString('en-IN')} />
                        <YAxis tick={{fontSize: 10, fill: '#6b7280', fontWeight: 'bold'}} tickLine={false} axisLine={{stroke: '#e5e7eb'}} tickFormatter={(val) => val >= 100000 || val <= -100000 ? `${(val/100000).toFixed(2)}L` : val} />
                        ...
                        <Area type="linear" dataKey="pl" stroke="url(#strokeColor)" strokeWidth={2.5} fill="url(#splitColor)" isAnimationActive={false} />
                        <Line type="monotone" dataKey="t0" stroke="#3b82f6" strokeWidth={2} strokeDasharray="4 4" fill="none" dot={false} isAnimationActive={false} />
                      </ComposedChart>
                   </ResponsiveContainer>
                </div>
             </div>
